...
public abstract class AbstractWebElement implements GUIElement {

    private static final Logger LOG = LoggerFactory.getLogger(AbstractWebElement.class);

    private final SelenideElement self;
    private final String typeHierarchy;
    private final String name;

    protected AbstractWebElement(Locators locator, TypeNames typeName, String name, ComplexGUIElement container) {
        this.self = findSelf(locator, name, container);
        this.typeHierarchy = container.toString() + " -> " + typeName.value();
        this.name = name;
    }

    protected  AbstractWebElement(SelenideElement self, TypeNames typeName) {
        this.self = self;
    ...
    private SelenideElement findSelf(Locators locator, String name, ComplexGUIElement container) {
        ComplexWebElement complexWebElement;
        if(container instanceof ComplexWebElement){
            complexWebElement = (ComplexWebElement) container;
        }else{
            throw new TestFrameworkException("Error while handling container for element '" + name
                    + "'\n Expected: " + ComplexWebElement.class.getName() + " Actual: " + container.getClass().getName());
        }
